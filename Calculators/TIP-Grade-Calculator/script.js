...
function updateGrade() {
    ...
    grades[currentPeriod]["Class Standing"] = {"Grade": 0, "Modified": grades[currentPeriod]["Class Standing"]["Modified"]}
...
function loadPeriod() {
    Object.keys(grades[currentPeriod]["Class Standing"]).forEach((factor) => {
        if (factor !== "Grade" && factor !== "Modified") {
...
function gradePeriod(clickedNode) {    
    ...
    currentPeriod = clickedNode.innerHTML  // Get current period (Prelim, Midterm, Final)
    CSContainer.innerHTML = ''  // Reset Class Standing Container
    majorExamGrade.value = 0  // Reset Major Exam Grade
    loadPeriod()
    console.log(grades)
}

function addCategory() {
    var categoriesCount = CSContainer.childNodes.length + 1
    ...
    const categoryName = document.createElement('input')
    categoryName.className = "category-name"
    categoryName.value = "CS-" + categoriesCount
    categoryName.type = "text"
    categoryName.placeholder = "*Name*"
    ...
    percentage.className = "percentage"
    percentage.placeholder = "*Percentage*"
    percentage.addEventListener('input', function(event) {
        updatePercentage(factorDiv, event.target.value)
    });
    ...
    updateCategories(categoriesCount)
...
function addSubCategory(node) {
    ...
    updatePercentage(parentNode, percentage)
...
function deleteCategory(node) {
    ...
    updateCategories(CSContainer.childNodes.length + 1)
}
...
function updateCategories(categoriesCount) {
    if (currentPeriod === "Prelim") {
        grades["Prelim"]["Class Standing"]["CS-" + categoriesCount] = {}
        if (!grades["Midterm"]["Class Standing"]["Modified"]) {
            grades["Midterm"]["Class Standing"]["CS-" + categoriesCount] = {}
        }
        if (!grades["Final"]["Class Standing"]["Modified"]) {
            grades["Final"]["Class Standing"]["CS-" + categoriesCount] = {}
        }
    } else {
        grades[currentPeriod]["Class Standing"]["Modified"] = true
        grades[currentPeriod]["Class Standing"]["CS-" + categoriesCount] = {}
    }
}

function updatePercentage(parentNode, percentage) {
    if (currentPeriod === "Prelim") {
